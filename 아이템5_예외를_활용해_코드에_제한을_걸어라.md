...
따라서 이를 활용해서 타입 비교를 했다면, 스마트 캐스트가 작동한다. 다음 예에서는 어떤 사람(person)의 복장(person.out)이 드레스(dress)여야 코드가 정상적으로 진행된다. 따라서 만약 이러한 outfit 프로퍼티가 final이라면, outfit 프로퍼티가 Dress로 스마트 캐스트 된다.

```kotlin
fun changeDress(person: Person) {
	require(person.outfit is Dress) // 스마트 캐스트
...
이러한 경우  requireNotNull, checkNotNull이라는 특수한 함수를 사용해도 괜찮다. 둘 다 스마트 캐스트를 지원하므로, 변수를 언팩하는 용도로 활용할 수 있다.

```kotlin
class Person(val email: String?) {

	fun validateEmail(emaii: String?) { /*...*/ }
...
nullability를 목적으로, 오른쪽에 throw 또는 return을 두고 Elvis 연산자를 활용하는 경우가 많다. 이러한 코드는 굉장히 읽기 쉽고, 유연하게 사용할 수 있다. 첫번째로 오른쪽에 return을 넣으면, 오류를 발생시키지도 않고 단순하게 함수를 중지할 수도 있다.
...
프로퍼티에 문제가 있어서 null일 때 여러 처리를 해야 할 때도, return/throw와 run 함수를 조합해서 활용하면 된다. 이는 함수가 중지된 이유를 로그에 출력해야 할 때 사용할 수 있다.
...
이처럼 return과 throw를 활용한 Elvis 연산자는 nullable을 확인할 때 굉장히 많이 사용되는 관용적인 방법이다. 따라서 적극적으로 활용하는 것이 좋다. 또한 이런 코드는 함수의 앞부분에 넣어서 잘 보이게 만드는 것이 좋다.
...
이외에도 다른 오류들을 발생시킬 때 throw를 활용할 수도 있다.